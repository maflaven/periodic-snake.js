(function () {
  ...
  Game.prototype.addApple = function () {
    if (Math.random() > 0.5 && this.badApples.length < Game.MAX_BAD_APPLES) {
      var badApple = new PSnake.Apple({
        game: this,
        bad: true,
        color: "#333300",
        pos: PSnake.Util.randomPos(this.snake.minMaxY, [0, Game.DIM_Y])
      });

      this.badApples.push(badApple);
    } else if (this.goodApple.length < Game.MAX_GOOD_APPLES) {
