(function () {
  ...
  // Returns an offset position from the given object and offset value.
  var periodicOffset = Util.periodicOffset = function (obj, ord) {
    var timeOffset = PSnake.Snake.RADIUS * 2;
    if (ord) {
      timeOffset *= obj.ord;
    }
  ...
  var drawStar = Util.drawStar = function (ctx, color, cx, cy, spikes, outerRadius, innerRadius) {
    var rot = Math.PI / 2 * 3;
    var x = cx;
    var y = cy;
